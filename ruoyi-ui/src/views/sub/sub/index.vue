...
<script>
import { listSub, getSub, delSub, addSub, updateSub } from "@/api/sub/sub";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除课题维护数据项？').then(function() {
        return delSub(ids);
      }).then(() => {
        this.getList();
